    ...
    var LockScreen = {
        ...
        buildTemplate: function (userinfo) {
            return [
                ...
                '<div class="card-body">',
                '<form>',
                '<div class="current-order text-center">',
                '<img id="LockUserPicture" src="{{picture}}" alt="John Doe" class="rounded-circle order-image" />',
                '<h2 id="LockUserName" class="order-name text-dark m-0">{{username}}</h2>',
                '<p  id="LockUserEmail" class="order-email m-0">{{email}}</p>',
                '</div>',
