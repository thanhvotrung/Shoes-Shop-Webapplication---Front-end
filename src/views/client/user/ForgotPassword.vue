...
export default {
  ...
  data() {
    ...
    const schema2 = Yup.object().shape({
      ...
      confirmnewPassword: Yup.string().required("Mật khẩu không được trống.")
...
<!--                  </button>-->
                  ...
                  <Form @submit="sendNewPassword" v-if="showNewPass"  :validation-schema="schema2" v-slot="{ errors }">
                    <fieldset>
                      <Field name="newPassword" v-model="newPassword" type="text" placeholder="Xác nhận mật khẩu"
                             class="input"/>
                      <div class="text-danger font-italic text-1 text-end">{{ errors.newPassword }}</div>
                      <Field name="confirmnewPassword" v-model="confirmnewPassword"  type="text" placeholder="Xác nhận mật khẩu"
                            class="input"/>
                      <div class="text-danger font-italic text-1 text-end">{{ errors.confirmnewPassword }}</div>
                      <div class="box">
                        <button  type="submit"  class="btn-type1">Đổi mật khẩu</button>
                      </div>
                    </fieldset>
                  </Form>
